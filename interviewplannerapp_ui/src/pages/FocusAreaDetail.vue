					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateFocusArea()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2FocusAreas = true">Add</base-button>
								              <modal :show.sync="modal2FocusAreas">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Focus Area Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="FocusAreaId" type="text" placeholder="Enter FocusAreaId" v-model="focusAreaDetails.focusAreaId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Name</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Name" type="text" placeholder="Enter Name" v-model="focusAreaDetails.name"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async updateFocusArea() {
      ...
      const jsonData = JSON.stringify(this.focusAreaDetails);
      const res = await FocusAreaService.update(jsonData);
...
//        this.getAllFocusAreas();
    ...
    async getFocusAreaDetails() {
      try {
			let response = await FocusAreaService.get(this.focusAreaId);
			this.focusAreaDetails = response.data;
