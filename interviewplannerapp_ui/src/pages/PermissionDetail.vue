					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updatePermission()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2Permissions = true">Add</base-button>
								              <modal :show.sync="modal2Permissions">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Permission Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="PermissionId" type="text" placeholder="Enter PermissionId" v-model="permissionDetails.permissionId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Type</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Type" type="text" placeholder="Enter Type" v-model="permissionDetails.type"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async updatePermission() {
      ...
      const jsonData = JSON.stringify(this.permissionDetails);
      const res = await PermissionService.update(jsonData);
...
//        this.getAllPermissions();
    ...
    async getPermissionDetails() {
      try {
			let response = await PermissionService.get(this.permissionId);
			this.permissionDetails = response.data;
