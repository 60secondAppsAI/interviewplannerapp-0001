					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateQuestion()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2Questions = true">Add</base-button>
								              <modal :show.sync="modal2Questions">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Question Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="QuestionId" type="text" placeholder="Enter QuestionId" v-model="questionDetails.questionId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Content</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Content" type="text" placeholder="Enter Content" v-model="questionDetails.content"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async updateQuestion() {
      ...
      const jsonData = JSON.stringify(this.questionDetails);
      const res = await QuestionService.update(jsonData);
...
//        this.getAllQuestions();
    ...
    async getQuestionDetails() {
      try {
			let response = await QuestionService.get(this.questionId);
