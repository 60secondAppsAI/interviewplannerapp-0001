					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updateInterviewer()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2Interviewers = true">Add</base-button>
								              <modal :show.sync="modal2Interviewers">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Interviewer Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="InterviewerId" type="text" placeholder="Enter InterviewerId" v-model="interviewerDetails.interviewerId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Name</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Name" type="text" placeholder="Enter Name" v-model="interviewerDetails.name"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Expertise</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="Expertise" type="text" placeholder="Enter Expertise" v-model="interviewerDetails.expertise"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async updateInterviewer() {
      ...
      const jsonData = JSON.stringify(this.interviewerDetails);
      const res = await InterviewerService.update(jsonData);
...
//        this.getAllInterviewers();
    ...
    async getInterviewerDetails() {
      try {
			let response = await InterviewerService.get(this.interviewerId);
			this.interviewerDetails = response.data;
