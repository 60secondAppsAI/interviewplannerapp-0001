        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalMeetingNotes = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalMeetingNotes">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add MeetingNote</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="MeetingNoteId" type="text" placeholder="Enter MeetingNoteId" v-model="meetingNoteToAdd.meetingNoteId"></base-input>
  <base-input label="Text" type="text" placeholder="Enter Text" v-model="meetingNoteToAdd.text"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="meetingNotes" :row-key="record => record.MeetingNoteId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <MeetingNotePictureView :meetingNotes="meetingNotes" />
          </div>
...
import MeetingNotePictureView from './MeetingNotePictureView.vue';


const meetingNotesColumns = [
  "meetingNoteId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    MeetingNotePictureView
  },

  data() {
    return {
      modalMeetingNotes: false,
        isTableView: true,

      columns: [
        {
          title: 'Meeting Note Id',
		dataIndex: 'meetingNoteId',
          visible: true,
          scopedSlots: { customRender: 'meetingNoteId' },
          sorter: true
          //sorter: (a, b) => a.meetingNoteId - b.meetingNoteId,
          //sorter: (a, b) => a.meetingNoteId.localeCompare(b.meetingNoteId),
        },
        {
          title: 'Text',
		dataIndex: 'text',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} meetingNotes`,
      },
      ...
      meetingNotesTable: {
        columns: [...meetingNotesColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'meetingNoteId',           // Column to sort by
  ...
  methods: {
    ...
    async renderMeetingNotesTable() {
      ...
      let meetingNotesTableData = [];
      for (let i = 0; i < this.meetingNotes.length; i++) {
        meetingNotesTableData.push({
          id: i,
          meetingNoteId: this.meetingNotes[i].meetingNoteId,
          year: this.meetingNotes[i].year,
          date: this.meetingNotes[i].date,
          competitionId: this.meetingNotes[i].competitionId,
          meetingNoteId: this.meetingNotes[i].meetingNoteId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-meeting-notes',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForMeetingNotesChanged', this.searchQuery);
		//this.renderMeetingNotesTable();
    },
	...
	async handleAddSubmitted() {
      this.modalMeetingNotes = false;

      const currentDate = new Date().getTime();
      this.meetingNoteToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.meetingNoteToAdd);
      console.log(jsonData);
      
      const res = await MeetingNoteService.addMeetingNote(jsonData);
