...
<template>
        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Documentations</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalDocumentations = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalDocumentations">
                <template slot="header">
  ...
  <base-input label="Link" type="text" placeholder="Enter Link" v-model="documentationToAdd.link"></base-input>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="documentations" :row-key="record => record.DocumentationId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <DocumentationPictureView :documentations="documentations" />
          </div>
...
import DocumentationPictureView from './DocumentationPictureView.vue';


const documentationsColumns = [
  "documentationId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    DocumentationPictureView
  },

  data() {
    return {
      modalDocumentations: false,
        isTableView: true,

      columns: [
        {
          title: 'Documentation Id',
		dataIndex: 'documentationId',
          visible: true,
          scopedSlots: { customRender: 'documentationId' },
          sorter: true
          //sorter: (a, b) => a.documentationId - b.documentationId,
          //sorter: (a, b) => a.documentationId.localeCompare(b.documentationId),
        },
        {
          title: 'Name',
		dataIndex: 'name',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} documentations`,
      },
      ...
      documentationsTable: {
        columns: [...documentationsColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'documentationId',           // Column to sort by
  ...
  methods: {
    ...
    async renderDocumentationsTable() {
      ...
      let documentationsTableData = [];
      for (let i = 0; i < this.documentations.length; i++) {
        documentationsTableData.push({
          id: i,
          documentationId: this.documentations[i].documentationId,
          year: this.documentations[i].year,
          date: this.documentations[i].date,
          competitionId: this.documentations[i].competitionId,
          documentationId: this.documentations[i].documentationId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-documentations',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForDocumentationsChanged', this.searchQuery);
		//this.renderDocumentationsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalDocumentations = false;

      const currentDate = new Date().getTime();
      this.documentationToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.documentationToAdd);
      console.log(jsonData);
      
      const res = await DocumentationService.addDocumentation(jsonData);
