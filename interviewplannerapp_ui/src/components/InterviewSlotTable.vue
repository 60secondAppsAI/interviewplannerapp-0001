...
<template>
        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Interview Slots</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalInterviewSlots = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalInterviewSlots">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add InterviewSlot</h5>
                </template>
                <div>
                  <form @submit.prevent>
  ...
  <base-input label="EndTime" type="text" placeholder="Enter EndTime" v-model="interviewSlotToAdd.endTime"></base-input>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="interviewSlots" :row-key="record => record.InterviewSlotId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <InterviewSlotPictureView :interviewSlots="interviewSlots" />
          </div>
...
import InterviewSlotPictureView from './InterviewSlotPictureView.vue';


const interviewSlotsColumns = [
  "interviewSlotId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    InterviewSlotPictureView
  },

  data() {
    return {
      modalInterviewSlots: false,
        isTableView: true,

      columns: [
        {
          title: 'Interview Slot Id',
		dataIndex: 'interviewSlotId',
          visible: true,
          scopedSlots: { customRender: 'interviewSlotId' },
          sorter: true
          //sorter: (a, b) => a.interviewSlotId - b.interviewSlotId,
          //sorter: (a, b) => a.interviewSlotId.localeCompare(b.interviewSlotId),
        },
        {
          title: 'Start Time',
		dataIndex: 'startTime',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} interviewSlots`,
      },
      ...
      interviewSlotsTable: {
        columns: [...interviewSlotsColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'interviewSlotId',           // Column to sort by
  ...
  methods: {
    ...
    async renderInterviewSlotsTable() {
      ...
      let interviewSlotsTableData = [];
      for (let i = 0; i < this.interviewSlots.length; i++) {
        interviewSlotsTableData.push({
          id: i,
          interviewSlotId: this.interviewSlots[i].interviewSlotId,
          year: this.interviewSlots[i].year,
          date: this.interviewSlots[i].date,
          competitionId: this.interviewSlots[i].competitionId,
          interviewSlotId: this.interviewSlots[i].interviewSlotId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-interview-slots',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForInterviewSlotsChanged', this.searchQuery);
		//this.renderInterviewSlotsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalInterviewSlots = false;

      const currentDate = new Date().getTime();
      this.interviewSlotToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.interviewSlotToAdd);
      console.log(jsonData);
      
      const res = await InterviewSlotService.addInterviewSlot(jsonData);
