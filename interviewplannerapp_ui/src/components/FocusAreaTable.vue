...
<template>
        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Focus Areas</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalFocusAreas = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalFocusAreas">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add FocusArea</h5>
                </template>
                <div>
                  <form @submit.prevent>
  ...
  <base-input label="Name" type="text" placeholder="Enter Name" v-model="focusAreaToAdd.name"></base-input>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="focusAreas" :row-key="record => record.FocusAreaId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <FocusAreaPictureView :focusAreas="focusAreas" />
          </div>
...
import FocusAreaPictureView from './FocusAreaPictureView.vue';


const focusAreasColumns = [
  "focusAreaId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    FocusAreaPictureView
  },

  data() {
    return {
      modalFocusAreas: false,
        isTableView: true,

      columns: [
        {
          title: 'Focus Area Id',
		dataIndex: 'focusAreaId',
          visible: true,
          scopedSlots: { customRender: 'focusAreaId' },
          sorter: true
          //sorter: (a, b) => a.focusAreaId - b.focusAreaId,
          //sorter: (a, b) => a.focusAreaId.localeCompare(b.focusAreaId),
        },
        {
          title: 'Name',
		dataIndex: 'name',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} focusAreas`,
      },
      ...
      focusAreasTable: {
        columns: [...focusAreasColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'focusAreaId',           // Column to sort by
  ...
  methods: {
    ...
    async renderFocusAreasTable() {
      ...
      let focusAreasTableData = [];
      for (let i = 0; i < this.focusAreas.length; i++) {
        focusAreasTableData.push({
          id: i,
          focusAreaId: this.focusAreas[i].focusAreaId,
          year: this.focusAreas[i].year,
          date: this.focusAreas[i].date,
          competitionId: this.focusAreas[i].competitionId,
          focusAreaId: this.focusAreas[i].focusAreaId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-focus-areas',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForFocusAreasChanged', this.searchQuery);
		//this.renderFocusAreasTable();
    },
	...
	async handleAddSubmitted() {
      this.modalFocusAreas = false;

      const currentDate = new Date().getTime();
      this.focusAreaToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.focusAreaToAdd);
      console.log(jsonData);
      
      const res = await FocusAreaService.addFocusArea(jsonData);
