...
<template>
        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalSkills = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalSkills">
                <template slot="header">
  ...
  <base-input label="Level" type="text" placeholder="Enter Level" v-model="skillToAdd.level"></base-input>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="skills" :row-key="record => record.SkillId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <SkillPictureView :skills="skills" />
          </div>
...
import SkillPictureView from './SkillPictureView.vue';
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'skillId',
          visible: true,
          scopedSlots: { customRender: 'skillId' },
          sorter: true
          //sorter: (a, b) => a.skillId - b.skillId,
          //sorter: (a, b) => a.skillId.localeCompare(b.skillId),
        },
        {
          title: 'Name',
		dataIndex: 'name',
      ...
      sortBy: 'skillId',           // Column to sort by
  ...
  methods: {
    ...
    async renderSkillsTable() {
      ...
      for (let i = 0; i < this.skills.length; i++) {
        skillsTableData.push({
          ...
          year: this.skills[i].year,
          date: this.skills[i].date,
          competitionId: this.skills[i].competitionId,
          skillId: this.skills[i].skillId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-skills',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForSkillsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      ...
      this.skillToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.skillToAdd);
      console.log(jsonData);
      
      const res = await SkillService.addSkill(jsonData);
