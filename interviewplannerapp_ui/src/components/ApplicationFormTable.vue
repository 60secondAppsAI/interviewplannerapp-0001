        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalApplicationForms = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalApplicationForms">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add ApplicationForm</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="ApplicationFormId" type="text" placeholder="Enter ApplicationFormId" v-model="applicationFormToAdd.applicationFormId"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="applicationForms" :row-key="record => record.ApplicationFormId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <ApplicationFormPictureView :applicationForms="applicationForms" />
          </div>
...
import ApplicationFormPictureView from './ApplicationFormPictureView.vue';


const applicationFormsColumns = [
  "applicationFormId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  components: {
    ...
    Pagination,
    ApplicationFormPictureView
  },

  data() {
    return {
      modalApplicationForms: false,
        isTableView: true,

      columns: [
        {
          title: 'Application Form Id',
		dataIndex: 'applicationFormId',
          visible: true,
          scopedSlots: { customRender: 'applicationFormId' },
          sorter: true
          //sorter: (a, b) => a.applicationFormId - b.applicationFormId,
          //sorter: (a, b) => a.applicationFormId.localeCompare(b.applicationFormId),
        },
      ],
      pagination: {
        current: 1,
        ...
        showTotal: (total) => `Total ${total} applicationForms`,
      },

      applicationForms: [],
      applicationFormToAdd: {},

      applicationFormsTable: {
        columns: [...applicationFormsColumns],
        data: [],
      },

      // New properties for sorting and searching
      sortBy: 'applicationFormId',           // Column to sort by
  ...
  methods: {
    ...
    async renderApplicationFormsTable() {
      ...
      let applicationFormsTableData = [];
      for (let i = 0; i < this.applicationForms.length; i++) {
        applicationFormsTableData.push({
          id: i,
          applicationFormId: this.applicationForms[i].applicationFormId,
          year: this.applicationForms[i].year,
          date: this.applicationForms[i].date,
          competitionId: this.applicationForms[i].competitionId,
          applicationFormId: this.applicationForms[i].applicationFormId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-application-forms',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForApplicationFormsChanged', this.searchQuery);
		//this.renderApplicationFormsTable();
    },
	...
	async handleAddSubmitted() {
      this.modalApplicationForms = false;

      const currentDate = new Date().getTime();
      this.applicationFormToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.applicationFormToAdd);
      console.log(jsonData);
      
      const res = await ApplicationFormService.addApplicationForm(jsonData);
